...
本文件记录每个版本实际完成的功能、修复和验证结果。每个 `vX.Y.Z` 章节必须与同名 Git tag 对应：版本号一致，tag 中列出的每项主要更新都必须在章节中出现，章节不得混入其他版本内容。同一版本号后续追加功能时，章节和 tag 必须累计该版本的全部主要更新，不能覆盖既有记录；只有发布新版本号时才创建新的章节和 tag。发布前必须逐项核对 `git show vX.Y.Z` 与本文件；Git tag 由项目维护者手动创建。
